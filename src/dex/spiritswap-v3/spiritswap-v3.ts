...
import {
  ...
  PoolState,
  SpiritswapV3Data,
  SpiritswapV3Functions,
  SpiritswapV3Param,
  SpiritswapV3SimpleSwapParams,
} from './types';
import {
  getLocalDeadlineAsFriendlyPlaceholder,
  SimpleExchange,
} from '../simple-exchange';
import { Adapters, PoolsToPreload, SpiritswapV3Config } from './config';
...
export class SpiritSwapV3
  ...
  implements IDex<SpiritswapV3Data, SpiritswapV3Param>
{
  private readonly factory: SpiritswapV3Factory;
  readonly isFeeOnTransferSupported: boolean = false;
  readonly eventPools: Record<string, SpiritswapV3EventPool | null> = {};
  ...
  public static dexKeysWithNetwork: { key: string; networks: Network[] }[] =
    getDexKeysWithNetwork(_.pick(SpiritswapV3Config, ['SpiritSwapV3']));
  ...
  constructor(
    ...
    protected config = SpiritswapV3Config[dexKey][network],
    ...
    this.factory = new SpiritswapV3Factory(
      dexHelper,
      dexKey,
      this.config.factory,
      this.logger,
  ...
  async getPricingFromRpc(
    ...
    amounts: bigint[],
    side: SwapSide,
    pools: SpiritswapV3EventPool[],
    states: PoolState[],
  ): Promise<ExchangePrices<SpiritswapV3Data> | null> {
    if (pools.length === 0) {
      return null;
    }
  ...
  async getPricesVolume(
    ...
    blockNumber: number,
    limitPools?: string[],
  ): Promise<null | ExchangePrices<SpiritswapV3Data>> {
    try {
      const _srcToken = this.dexHelper.config.wrapETH(srcToken);
      const _destToken = this.dexHelper.config.wrapETH(destToken);
      ...
      const poolsToUse = selectedPools.reduce(
        ...
        {
          poolWithState: [] as SpiritswapV3EventPool[],
          poolWithoutState: [] as SpiritswapV3EventPool[],
        },
      );
      ...
      const notNullResult = result.filter(
        res => res !== null,
      ) as ExchangePrices<SpiritswapV3Data>;
  ...
  getCalldataGasCost(
    poolPrices: PoolPrices<SpiritswapV3Data>,
  ): number | number[] {
    const gasCost =
  ...
  async preProcessTransaction(
    optimalSwapExchange: OptimalSwapExchange<SpiritswapV3Data>,
    srcToken: Token,
    _0: Token,
    _1: SwapSide,
    options: PreprocessTransactionOptions,
  ): Promise<[OptimalSwapExchange<SpiritswapV3Data>, ExchangeTxInfo]> {
  ...
  getDirectParam(
    ...
    const swapParams: SpiritswapV3Param = [
      srcToken,
      destToken,
      this.config.router,
      srcAmount,
    ...
    const encoder = (...params: SpiritswapV3Param) => {
  ...
  async getSimpleParam(
    ...
    const path = this._encodePath(data.path, side);
    const swapFunctionParams: SpiritswapV3SimpleSwapParams =
      side === SwapSide.SELL
        ? {
