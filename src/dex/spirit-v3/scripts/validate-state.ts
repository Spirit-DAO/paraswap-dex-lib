...
async function _fetchPoolStateSingleStep(
  blockNumber: number,
...
async function isPoolStateEqualToReal(state: PoolState, blockNumber: number) {
  const [balance0, balance1, contractState] = await _fetchPoolStateSingleStep(
    blockNumber,
  );
...
async function checkPoolStateForBlockRange(
  ...
  const algebra = new SpiritSwapV3(network, dexKey, dexHelper);
  ...
  const state = pool?.getState(startBlockNumber) as PoolState;
